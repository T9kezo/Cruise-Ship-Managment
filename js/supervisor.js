...
window.viewOrders = async function(status = null) {
    try {
        ...
        title.textContent = status ? `Stationery Orders - ${status.charAt(0).toUpperCase() + status.slice(1)}` : 'All Stationery Orders';
        ...
        logger.info(`Viewing stationery orders: ${orders.length} found`);
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', async () => {
    logger.info('Supervisor dashboard loaded');
